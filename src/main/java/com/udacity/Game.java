...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for (int i = 0; i < player.length; i++) {
            if (grid[0][0] == player[i] && grid[1][1] == player[i] && grid[2][2] == player[i]) {
                return message[i];
            }
            if (grid[0][2] == player[i] && grid[1][1] == player[i] && grid[2][0] == player[i]) {
                return message[i];
            }
            for (int j = 0; j < grid.length; j++) {
                if (grid[0][j] == player[i] && grid[1][j] == player[i] && grid[2][j] == player[i]) {
                    return message[i];
                }
                if (grid[j][0] == player[i] && grid[j][1] == player[i] && grid[j][2] == player[i]) {
                    return message[i];
                }
            }
        }
